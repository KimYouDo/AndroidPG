...
public class ManualEntryActivity extends ListActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        entry.setInputType(bundle.getInt(StartFragment.INPUT_TYPE,0)); // 입력 방법
        entry.setActivityType(bundle.getInt(StartFragment.ACTIVITY_TYPE, 0));  // 운동 종류

        // Set time and date
        mDateAndTime = Calendar.getInstance();    // 현재 일시.
        entry.setDateTime(mDateAndTime.getTimeInMillis());
    ...
    public void selectManualSave(View v) {
        ...
        // ExerciseEntry를 데이터베이스에 저장한다.
        new WriteToDB().execute(entry);
    ...
    private class WriteToDB extends AsyncTask<ExerciseEntry, Void, String> {
        ...
        protected String doInBackground(ExerciseEntry... exerciseEntries) {
            ...
            long id = MainActivity.DBhelper.insertEntry(exerciseEntries[0]);
            return ""+id;
        }

        // doInBackground 메소드가 리턴한 후 호출되는 메소드로서
        // UI 스레드에 의해 실행됨.
        @Override
        protected void onPostExecute(String result) {
            // ExerciseEntry가 DB에 저장됐음을 사용자에게 알림.
